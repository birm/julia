    ...
    if (jl_is_datatype(a) && jl_is_datatype(b)) {
        ...
        for (i = 0; i < np; i++) {
            ...
            if (jl_is_typevar(ai) || jl_is_typevar(bi))
                continue; // Union{} is always in this intersection
            if (jl_is_type(ai)) {
                if (jl_is_type(bi)) {
...
int does_subtypes_concrete(jl_value_t *a)
{
    if (jl_is_unionall(a))
        a = jl_unwrap_unionall(a);
    if (jl_is_typevar(a))
        return 1; // (maybe)
    if (jl_is_uniontype(a))
        return does_subtypes_concrete(((jl_uniontype_t*)a)->a) ||
               does_subtypes_concrete(((jl_uniontype_t*)a)->b);
    if (jl_is_vararg_type(a))
        return does_subtypes_concrete(jl_tparam0(a));
    ...
    if (jl_is_datatype(a)) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (tpl && p == jl_bottom_type)
                return 1;
            if (tpl && does_subtypes_concrete(p))
                return 1;
        }
    }
    return 0;
    ...
    *ans = jl_bottom_type;
    ...
    if (jl_subtype_env(a, b, env, szb)) {
        ...
        *ans = b;
        ...
        if (lta && !does_subtypes_concrete(b))
            goto bot;
        if (ltb && !does_subtypes_concrete(a))
            goto bot;
        jl_stenv_t e;
        init_stenv(&e, NULL, 0);
        e.intersection = e.ignore_free = 1;
